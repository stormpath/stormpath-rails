...
shared_examples "stormpath account" do
  context "after initialize" do
    let(:account) { mock("account", href: "account_href") }
    ...
    context "on find" do
      ...
      it "should setup object with data from stormpath" do
        ...
        (subject.class::STORMPATH_FIELDS - [:password]).each do |field_name|
          found.send(field_name).should == account.send("#{field_name}")
        end
      end
  ...
  context "before create" do
    it "should create account at stormpath and assign stormpath_url" do
      Stormpath::Rails::Client.should_receive(:create_account!).and_return(mock("account", href: "account_href"))
      subject.save!
